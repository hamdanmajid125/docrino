...
(function ($) {
    ...
    $('.target').on('change', function () {
        ...
        if ($('#doctor').val() != 0) {
            ...
            $('#help-block p').text('Please Select Doctor First');
        }
    ...
    function AddAppointment(date) {
        ...
        $.ajax({
            url: baseURL + '/appointment/checkslots',
            type: 'POST',
            cache: false,
            data: {
                doctor: $('#doctor').val(),
            ...
            success: function (response) {
                if(response.status){
                    var options = '';
                    $.each(response.final_slots, function (key, value) {
                    ...
                    $('#RDVModalSubmit').on('show.bs.modal', function (event) {
                        ...
                        var rdv_time_end = button.data('rdv_time_end') // Extract info from data-* attributes
                        var selectedPatientID = $("#patient_name").val() // Extract info from data-* attributes
                        var selectedDoctorID = $("#doctor").val() // Extract info from data-* attributes
                        var selectedPatientName = $("#patient_name option:selected").text() // Extract info from data-* attributes
                        var selectedDoctorName = $("#doctor option:selected").text() // Extract info from data-* attributes
                        ...
                        if (selectedPatientID == 0) {
                            ...
                            modal.find('#rdv_doctor').html('<span class="text-danger"><b>Select Patient Before Submitting</b></span>');
                        }
                        else {
                            modal.find('#patient_name').text(selectedPatientName)
                            modal.find('#rdv_doctor').text(selectedDoctorName)
                        }
                        modal.find('#rdv_date').text(rdv_date)
                        ...
                        modal.find('#rdv_time_end_input').val(rdv_time_end)
                        modal.find('#patient_input').val(selectedPatientID)
                        modal.find('#doctor_input').val(selectedDoctorID)
